...
//  ProgLabel_1.1.1  Create string with version info
  !!version_info+="SS-V3.24V-safe;_03/09/2015;_Stock_Synthesis_by_Richard_Methot_(NOAA)_using_ADMB_11.1x64";
  !!version_info_short+="#V3.24V";
...
//  ProgLabel_5.0.1  setup convergence critera and max func evaluations
 LOCAL_CALCS
    ...
    ad_comm::change_pinfile_name("canary.par");}
            ...
            case 3:          // hybrid F_method
            {
            	...
            	for(int tune_F=1;tune_F<=F_Tune;tune_F++)
              {
                ...
                if(tune_F<F_Tune)
                {
                  ...
                  Z_adjuster = totcatch_byarea(t,p)/(Z_adjuster2+0.0001);   // but this totcatch_byarea needs to exclude fisheries with F from param
                  for (g=1;g<=gmorph;g++)
                  if(use_morph(g)>0)
                  {
...
//  get expected age comp in the bins and with ageing error
        {
         ...
         if(yr_a_t(f,i)==t)
         {
          ...
          if(docheckup==1) echoinput<<" age obs2 "<<mkt_a(f,i)<<endl<<agetemp<<endl<<obs_a(f,i)<<endl<<exp_a(f,i)<<endl;
...
//      if(time_vary_sel(yz,f+Ntypes)>0 || time_vary_sel(yz,f)>0 || time_vary_MG(yz,2)>0 || time_vary_MG(yz,3)>0)
      if(time_vary_makefishsel(yf,f)>0 || save_for_report>0)
      {
        ...
        if(seltype(f,2)!=0)
        {
          sel_al_4(s,g,f)=elem_prod(sel_a(yf,f,gg),(ALK_w * sel_l_r(yf,f,gg) ));  //  retained numbers
          if(WTage_rd==0)
...
//            report5<<Tune_F<<" tune_area"<<Fcast_Catch_Current_Alloc<<endl;
          ...
          if(Fcast_Catch_Allocation_Groups>0  && y>=Fcast_Cap_FirstYear)  // adjust to get a specific fleet allocation
          {
            ...
            for(g=1;g<=Fcast_Catch_Allocation_Groups;g++)
            {
              temp2=(Fcast_Catch_Allocation(y-endyr,g)/temp1) / (Fcast_Catch_Current_Allocation(g)/temp);
              for(f=1;f<=Nfleet;f++)
              if (Allocation_Fleet_Assignments(f)==g)
              {
