...
system(paste0('parabola -noest -mcmc 1 -nuts -hyeps ', eps, ' -mcseed ',10), ignore.stdout=FALSE)
out1.admb <- cbind(i=i,read.table(file[1], sep=' ', row.names=NULL, header=TRUE)[,-1])
out2.admb <- cbind(i=i,read.table(file[2], sep=' ', row.names=NULL, header=TRUE))
out1.tmb <- data.frame(i=i,temp$out, row.names=NULL)
out2.tmb <- data.frame(cbind(i,temp$out2))
names(out2.tmb) <- names(out2.admb)
names(out1.tmb) <- names(out1.admb)
out1.tmb$treedepth <- factor(rep(0:10, times=2^(0:10))[1:nrow(out1.tmb)])
out1.admb$treedepth <- factor(rep(0:10, times=2^(0:10))[1:nrow(out1.admb)])
rm(out1)
out1 <- rbind(cbind(m='admb', out1.admb), cbind(m='tmb', out1.tmb))
out2 <- rbind(cbind(m='admb', out2.admb), cbind(m='tmb', out2.tmb))
g <- ggplot(data=out1) + facet_wrap('m')
g+ geom_point(aes(theta1, theta2, color=treedepth))
...
system(paste0('funnel -noest -mcmc 1 -nuts -hyeps ', eps, ' -mcseed ',10), ignore.stdout=FALSE)
out1.admb <- cbind(i=i,read.table(file[1], sep=' ', row.names=NULL, header=TRUE)[,-1])
out2.admb <- cbind(i=i,read.table(file[2], sep=' ', row.names=NULL, header=TRUE))
setwd('..')
out1.tmb <- data.frame(i=i,temp$out, row.names=NULL)
...
out1 <- rbind(cbind(m='admb', out1.admb), cbind(m='tmb', out1.tmb))
out2 <- rbind(cbind(m='admb', out2.admb), cbind(m='tmb', out2.tmb))
g <- ggplot(data=out1) + facet_wrap('m') + xlim(-5,5) + ylim(-5,5)
g+ geom_point(aes(theta1, theta2, color=treedepth))
#plot(out1.tmb$alpha, out1.tmb$alpha-out1.admb$alpha)
...
#set.seed(6)
setwd('models/mvnd/')
#stan.fit <- stan(file='mvnd.stan', data=list())
...
eps <- .1
Npar <- 2
covar <- matrix(c(1, .5,.5,1), 2)
obj <- MakeADFun(data=list(Npar=Npar, covar=covar), parameters=list(mu=x0), DLL='mvnd')
obj$env$beSilent()
fn2 <- function(x) -obj$fn(x)
gr2 <- function(x) -as.vector(obj$gr(x))
setwd('admb')
write.table(c(x0, r0, 10), file='input.txt', sep=' ', row.names=F,
            col.names=F)
write.table(c(Npar,covar), file='mvnd.dat', sep=' ', row.names=F,
            col.names=F)
 system('admb mvnd'); system('mvnd')
file <- c('trajectory.txt', 'out2.txt')
## trash <- lapply(file, function(f) if(file.exists(f)) file.remove(f))
temp <- run_mcmc.nuts.test(iter=1, warmup=1, fn=fn2, gr=gr2, init=x0, r0=r0,
                               chain=1, control=list(stepsize=eps))
## Be careful here with admodel.cov beine right!!
write.admb.cov(diag(2))
system(paste0('mvnd -noest -mcmc 1 -nuts -hyeps ', eps, ' -mcseed ',10), ignore.stdout=FALSE)
out1.admb <- cbind(i=i,read.table(file[1], sep=' ', row.names=NULL, header=TRUE)[,-1])
out2.admb <- cbind(i=i,read.table(file[2], sep=' ', row.names=NULL, header=TRUE))
setwd('..')
out1.tmb <- data.frame(i=i,temp$out, row.names=NULL)
