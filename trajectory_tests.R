...
zz <- yy <- list()
for(i in 1:10){
system(paste0('parabola -noest -mcmc 1 -nuts -hyeps ', eps, ' -mcseed ', i+10), ignore.stdout=T)
yy[[i]] <- cbind(i=i,read.table(file[1], sep=' ', row.names=NULL, header=TRUE)[,-1])
yy$treedepth <- factor(rep(0:10, times=2^(0:10))[1:nrow(yy[[i]])])
zz[[i]] <- cbind(i=i,read.table(file[2], sep=' ', row.names=NULL, header=TRUE))
}

zz <- do.call(rbind, zz)
subset(zz, j==4)$prob
subset(zz, j==4)$nprime
thetaprime[[i]] <- zz[nrow(zz), 5:6]
...
ggplot(yy, aes(thetaminus1, thetaminus2, color=treedepth)) + geom_point()
plot(yy$thetaminus1, ylim=range(yy$theta1), type='b')
points(yy$thetaplus1, type='b')
points(yy$theta1, type='b')
...
plot(alpha.tmb, (alpha.tmb-alpha.admb)/alpha.tmb, ylab=NA,
     ylim=1e-3*c(-1,1))
